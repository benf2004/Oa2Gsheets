...
// updates necessary stats
    ...
    const jumbo = decodeURI(urlParams.get("key"))
    ...
    async function sendInfo(token) {
        ...
        let refURL = `https://oa2gsheets.com/send.html?asin=${asin}&fba=${is_fba}&t=${encodeURIComponent(token)}&st=${st}&s_a=${ship_amz}&dy=${is_dynam}&top=${stats[15]}&drops=${stats[8]}&title=${enc_title}&cat=${enc_cat}&r=${stats[9]}&s=${ship}&other=${other}&fileID=${fileID}&o=${order}&cogs=${cogs}&sourceurl=${sourceURL}&refPer=${refPer}&price=${price}&notes=${notes}`;
